...
async def initialize_userbot(user_id): # this ensure the single startup .. even if logged in or not
    ...
    if data and data.get("session"):
        try:
            ...
            await app.send_message(user_id, "Please Login To the Bot Through Your Account")
            return None
    else:
        if DEFAULT_SESSION:
            return userrbot
...
async def batch_link(_, message):
    ...
    if users_loop.get(user_id, False):
        await app.send_message(
            message.chat.id,
            "Batch Already Processing Please Wait............"
        )
        return

    freecheck = await chk_user(message, user_id)
    if freecheck == 1 and FREEMIUM_LIMIT == 0 and user_id not in OWNER_ID and not await is_user_verified(user_id):
        await message.reply("Please Buy Premium First By Sending Msg On Telegram https://t.me/HRWALE007")
        return
    ...
    join_button = InlineKeyboardButton("Join Channel", url="https://t.me/scr007Botupdates")
    ...
    try:
        ...
        for i in range(cs, cs + cl):
            if not userbot:
                await app.send_message(message.chat.id, "Login in to the bot first and then try...")
                users_loop[user_id] = False
                return
            if user_id in users_loop and users_loop[user_id]:
                url = f"{'/'.join(start_id.split('/')[:-1])}/{i}"
...
async def stop_batch(_, message):
    ...
    if user_id in users_loop and users_loop[user_id]:
        ...
        await app.send_message(
            message.chat.id, 
            "Batch processing has been stopped successfully."
        )
    elif user_id in users_loop and not users_loop[user_id]:
        await app.send_message(
            message.chat.id, 
            "The batch process was already stopped."
        )
    else:
        await app.send_message(
            message.chat.id, 
